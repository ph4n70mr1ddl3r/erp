    ...
    async fn get_price_book(&self, pool: &SqlitePool, id: Uuid) -> Result<PriceBook> {
        ...
        Ok(PriceBook {
            base: erp_core::BaseEntity {
                ...
                created_by: erp_core::parse_uuid_opt(row.get::<Option<&str>, _>("created_by"), "created_by")?,
                updated_by: erp_core::parse_uuid_opt(row.get::<Option<&str>, _>("updated_by"), "updated_by")?,
            },
            ...
            valid_from: erp_core::parse_datetime_opt(row.get::<Option<&str>, _>("valid_from"), "valid_from")?,
            valid_to: erp_core::parse_datetime_opt(row.get::<Option<&str>, _>("valid_to"), "valid_to")?,
            parent_id: erp_core::parse_uuid_opt(row.get::<Option<&str>, _>("parent_id"), "parent_id")?,
        })
    }
        ...
        if let Some(row) = row {
            Ok(Some(PriceBookEntry {
                base: erp_core::BaseEntity {
                    ...
                    price_book_id: erp_core::parse_uuid(row.get::<&str, _>("price_book_id"), "price_book_id")?,
                    product_id: erp_core::parse_uuid(row.get::<&str, _>("product_id"), "product_id")?,
                    unit_price: row.get::<i64, _>("unit_price"),                currency: row.get::<&str, _>("currency").to_string(),
    ...
    async fn get_coupon(&self, pool: &SqlitePool, code: &str) -> Result<Option<Coupon>> {
        ...
        if let Some(row) = row {
            Ok(Some(Coupon {
                ...
                code: row.get::<&str, _>("code").to_string(),
                discount_id: erp_core::parse_uuid(row.get::<&str, _>("discount_id"), "discount_id")?,
                promotion_id: erp_core::parse_uuid_opt(row.get::<Option<&str>, _>("promotion_id"), "promotion_id")?,
                customer_id: erp_core::parse_uuid_opt(row.get::<Option<&str>, _>("customer_id"), "customer_id")?,
                is_used: row.get::<i32, _>("is_used") == 1,
                used_at: erp_core::parse_datetime_opt(row.get::<Option<&str>, _>("used_at"), "used_at")?,
                order_id: erp_core::parse_uuid_opt(row.get::<Option<&str>, _>("order_id"), "order_id")?,
                valid_from: erp_core::parse_datetime_opt(row.get::<Option<&str>, _>("valid_from"), "valid_from")?,
                valid_to: erp_core::parse_datetime_opt(row.get::<Option<&str>, _>("valid_to"), "valid_to")?,
                status: erp_core::Status::Active,
            }))
        ...
        for row in rows {
            tiers.push(PriceTier {
                base: erp_core::BaseEntity {
                    ...
                    price_book_entry_id: erp_core::parse_uuid_opt(row.get::<Option<&str>, _>("price_book_entry_id"), "price_book_entry_id")?,
                    product_id: erp_core::parse_uuid_opt(row.get::<Option<&str>, _>("product_id"), "product_id")?,
                    min_quantity: row.get::<i32, _>("min_quantity"),                max_quantity: row.get::<Option<i32>, _>("max_quantity"),
