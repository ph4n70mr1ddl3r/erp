...
impl ReportScheduleRepository for SqliteReportScheduleRepository {
    async fn create(&self, pool: &SqlitePool, schedule: &ReportSchedule) -> anyhow::Result<ReportSchedule> {
        ...
        .bind(schedule.cc_recipients.as_ref().map(|v| serde_json::to_string(v).ok()).flatten())
        .bind(schedule.bcc_recipients.as_ref().map(|v| serde_json::to_string(v).ok()).flatten())
        ...
        .bind(schedule.notification_recipients.as_ref().map(|v| serde_json::to_string(v).ok()).flatten())
        .bind(&schedule.status)
        .bind(schedule.priority)
        .bind(schedule.tags.as_ref().map(|v| serde_json::to_string(v).ok()).flatten())
