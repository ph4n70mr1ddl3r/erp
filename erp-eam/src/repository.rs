...
impl EamRepository {
    pub async fn create_equipment(pool: &SqlitePool, equipment: &EquipmentAsset) -> Result<()> {
        ...
        let meter_type = equipment.meter_type.as_ref().map(|t| serde_json::to_string(t)).transpose()?;
        ...
        match row {
            Some(r) => {
                let asset = EquipmentAsset {
                    ...
                    location_id: r.get::<Option<String>, _>("location_id").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    department_id: r.get::<Option<String>, _>("department_id").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    parent_asset_id: r.get::<Option<String>, _>("parent_asset_id").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    installation_date: r.get::<Option<String>, _>("installation_date").as_ref().map(|d| d.parse().ok()).flatten(),
                    warranty_end_date: r.get::<Option<String>, _>("warranty_end_date").as_ref().map(|d| d.parse().ok()).flatten(),
                    ...
                    useful_life_years: r.get::<Option<i32>, _>("useful_life_years").map(|v| v as i32),
                    ...
                    current_meter_reading: r.get::<Option<i64>, _>("current_meter_reading").map(|v| v as i64),
        ...
        match row {
            Some(r) => {
                let wo = WorkOrder {
                    ...
                    description: r.get("description"),
                    work_order_type: r.get::<Option<String>, _>("work_order_type").as_ref().map(|s| serde_json::from_str(s).ok()).flatten().unwrap_or(WorkOrderType::Corrective),
                    priority: r.get::<Option<String>, _>("priority").as_ref().map(|s| serde_json::from_str(s).ok()).flatten().unwrap_or(WorkOrderPriority::Medium),
                    asset_id: r.get::<Option<String>, _>("asset_id").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    location_id: r.get::<Option<String>, _>("location_id").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    failure_code_id: r.get::<Option<String>, _>("failure_code_id").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    ...
                    requested_by: r.get::<Option<String>, _>("requested_by").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    ...
                    assigned_to: r.get::<Option<String>, _>("assigned_to").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    assigned_team_id: r.get::<Option<String>, _>("assigned_team_id").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
                    ...
                    closed_by: r.get::<Option<String>, _>("closed_by").as_ref().map(|u| Uuid::parse_str(u).ok()).flatten(),
