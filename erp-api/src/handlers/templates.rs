...
pub async fn get_template(
    ...
    let template = service.get(&state.pool, id).await?
        .ok_or_else(|| erp_core::Error::NotFound("Template not found".into()))?;
...
pub async fn render_template(
    ...
    let template = if let Some(id) = req.template_id {
        ...
        return Err(crate::error::ApiError(erp_core::Error::validation("Either template_id or template_code is required".to_string())));
    };
    
    let template = template.ok_or_else(|| erp_core::Error::NotFound("Template not found".into()))?;
...
pub async fn render_email_template(
    ...
    let service = erp_templates::EmailTemplateService::new();
    let template = service.get(&state.pool, id).await?
        .ok_or_else(|| erp_core::Error::NotFound("Email template not found".into()))?;
